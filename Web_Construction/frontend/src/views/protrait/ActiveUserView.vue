...
<el-container style="margin: 20px;">
  ...
  <el-space v-show="timeData.time[0] !== timeData.time[1]" style="margin-left: 4%; margin-top: 30px">
    <el-card style="width: 940px;">
      <template #header>
        <p style="text-align: center; margin: 0px">每日热度指数时序图</p>
      </template>

      <div style="width: 100%"><TheLine :timeData="timeData" @send-data-date="changeDateData"></TheLine></div>
    </el-card>
  </el-space>
  ...
  <el-space v-show="timeData.time[0] === timeData.time[1]" style="margin-left: 4%; margin-top: 30px;">
    <el-card style="width: 940px;">
      ...
      <div style="float: left; width: 80%;"><TheMaleFemaleBar :data="timeData"></TheMaleFemaleBar></div>
  ...
  <el-space :size="18" style="margin-left: 4%; margin-top: 30px;">
    <el-card style="width: 700px;">
      <template #header>
        <p style="text-align: center; margin: 0px">{{ timeData.province }}用户最感兴趣的话题（Top10）</p>
      </template>
      <THeEmptyPie @send-topic-data="changeTopicData" :data=timeData.topic style="height: 426px"></THeEmptyPie>
    </el-card>
  ...
  <el-space :size="18" style="margin-left: 4%; margin-top: 30px;">
    <el-card style="width: 300px;">
      <template #header>
        <p style="text-align: center; margin: 0px">{{ timeData.province }}男用户（Top10）</p>
      </template>
      <el-table :data="timeData.sort.male" :show-header="false" height="350px">
        <el-table-column type="expand">
          <template #default="props">
            <p class="txt_p">id: {{ props.row.info.id }}</p>
            <p class="txt_p">用户名: {{ props.row.info.name }}</p>
            <p class="txt_p">IP地址: {{ props.row.info.location }}</p>
            <p class="txt_p">粉丝数: {{ props.row.info.follower }}</p>
            <p class="txt_p">关注数: {{ props.row.info.friend }}</p>
            <p class="txt_p">收藏数: {{ props.row.info.status }}</p>
          </template>
        </el-table-column>
    ...
    <el-card style="width: 300px;">
      <template #header>
        <p style="text-align: center; margin: 0px;">{{ timeData.province }}活跃用户（Top10）</p>
      </template>
      <el-table :data="timeData.sort.all" :show-header="false" height="350px">
        <el-table-column type="expand">
          <template #default="props">
            <p class="txt_p">id: {{ props.row.info.id }}</p>
            <p class="txt_p">用户名: {{ props.row.info.name }}</p>
    ...
    <el-card style="width: 300px;">
      <template #header>
        <p style="text-align: center; margin: 0px">{{ timeData.province }}女用户（Top10）</p>
      </template>
      <el-table :data="timeData.sort.female" :show-header="false" height="350px">
        <el-table-column type="expand">
          <template #default="props">
            <p class="txt_p">id: {{ props.row.info.id }}</p>
            <p class="txt_p">用户名: {{ props.row.info.name }}</p>
...
export default {
  ...
  methods: {
    ...
    async get_data() {
      ...
      this.$axios.get('http://127.0.0.1:8000/api/map', {
        params: {
          model: this.model,
        ...
        this.map2Data = res.map2
        // console.log(this.time_start)
        // console.log(this.time_end)
      })
      this.$axios.get('http://127.0.0.1:8000/api/time', {
        params: {
          model: this.model,
          time_start: this.time_start,
          time_end: this.time_end,
