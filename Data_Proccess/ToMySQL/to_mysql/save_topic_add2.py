...
def to_sql():
    try:
        sql = "insert into time_title_table ({}) values({})".format(col, the_sql)
        cursor.execute(sql)
        db.commit()
    except:
        print(sql)
...
if __name__ == '__main__':
    ...
    df = pd.read_csv('../../WeiBo/Data/to_database/topic_add3.csv')
    col = ['t_title', 't_label', 't_time', 't_host', 't_continue', 't_sort', 't_hot']
    df.columns = col
    col = ['ti_title', 'ti_label', 'ti_time', 'ti_hot']
    df = df[['t_title', 't_label', 't_time', 't_hot']]
